...
const _PANEL_TOOLTIP = (
  <div className="space-y-1.5">
    ...
    <ul className="space-y-0.5 list-none">
      <li>
        <span style={{ color: "oklch(0.82 0.19 195)" }}>Корневые узлы</span> —
        термины с наивысшим TF-IDF (топ 20%), ключевые понятия
      </li>
      <li>
        <span style={{ color: "oklch(0.78 0.19 75)" }}>Дочерние узлы</span> —
        термины, связанные с родительским понятием
      </li>
      <li>
        <span style={{ color: "oklch(0.65 0.22 300)" }}>TF-IDF</span> — числовая
        значимость термина в корпусе запросов
      </li>
    </ul>
    <p className="text-muted-foreground">
      Нажмите на узел со стрелкой, чтобы развернуть или свернуть ветку иерархии.
    </p>
  </div>
);
...
const TFIDF_TOOLTIP = (
  <div className="space-y-1">
    ...
    <p>
      Term Frequency–Inverse Document Frequency — мера важности термина. Высокое
      значение означает, что термин часто встречается в данной группе, но редко
      в других группах.
    </p>
  </div>
);
  ...
  return (
    <div>
      <button
        type="button"
        className={`
          w-full flex items-center gap-1.5 py-1 px-2 rounded cursor-pointer
          hover:bg-kg-panel/60 transition-colors group text-left
        `}
        ...
        {hasChildren ? (
          ...
          <span className="w-3 h-3 flex-shrink-0 flex items-center justify-center">
            <span
              ...
              style={{
                backgroundColor:
                  depth === 0 ? "oklch(0.82 0.19 195)" : "oklch(0.78 0.19 75)",
              }}
            />
          </span>
        )}
