...
import type { RelationType, Triplet } from "../types";
...
const RELATION_TOOLTIPS: Record<RelationType, React.ReactNode> = {
  association: (
    <div className="space-y-1">
      ...
      <p>
        Высокая совместная встречаемость терминов и схожие значения TF-IDF.
        Указывает на тематическую близость — термины относятся к одной
        предметной области.
      </p>
    </div>
  ),
  "co-occurrence": (
    <div className="space-y-1">
      ...
      <p>
        Термины часто появляются вместе в пределах одного контекстного окна (4
        слова). Статистическая связь без явной семантики.
      </p>
    </div>
  ),
  hierarchical: (
    <div className="space-y-1">
      ...
      <p>
        ...
        Отражает отношение «является частью» или «является видом».
      </p>
    </div>
  ),
  functional: (
    <div className="space-y-1">
      ...
      <p>
        ...
        Отражает отношение «используется для» или «влияет на».
      </p>
    </div>
  ),
};
...
const _PANEL_TOOLTIP = (
  <div className="space-y-1.5">
    ...
    <p>
      Семантические тройки вида «Субъект → Предикат → Объект», извлечённые из
      графа знаний.
    </p>
    <ul className="space-y-0.5 list-none">
      <li>
        <span style={{ color: "oklch(0.82 0.19 195)" }}>Субъект</span> —
        исходная сущность
      </li>
      <li>
        <span style={{ color: "oklch(0.78 0.19 75)" }}>Предикат</span> — тип
        связи между сущностями
      </li>
      <li>
        <span style={{ color: "oklch(0.65 0.22 300)" }}>Объект</span> — целевая
        сущность
      </li>
      <li>
        <span style={{ color: "oklch(0.72 0.2 145)" }}>Вес</span> — значимость
        связи (0–1)
      </li>
    </ul>
    <p className="text-muted-foreground">
      Используйте фильтры по типу связи и сортировку по весу для анализа
      наиболее значимых отношений.
    </p>
  </div>
);
...
const FILTER_TOOLTIP = (
  <div className="space-y-1">
    ...
    <p>
      Отображает только триплеты с выбранным типом предиката. «Все» — показывает
      все триплеты без фильтрации.
    </p>
  </div>
);
...
const SORT_TOOLTIP = (
  <div className="space-y-1">
    ...
    <p>
      Вес отражает силу связи между сущностями. Сортировка по убыванию (↓)
      показывает наиболее значимые отношения первыми.
    </p>
  </div>
);
...
export function TripletsPanel({ triplets }: TripletsPanelProps) {
  ...
  const filtered = useMemo(() => {
    let result =
      filterType === "all"
        ? triplets
        : triplets.filter((t) => t.predicate === filterType);
    return [...result].sort((a, b) =>
      sortDir === "desc" ? b.weight - a.weight : a.weight - b.weight,
    );
        ...
        <table className="w-full text-xs font-mono">
              ...
              <th className="text-left py-1.5 px-2 text-muted-foreground font-normal">
                <div className="flex items-center gap-1">
                  ...
                  <TooltipIcon
                    content={
                      <p>
                        Исходная сущность в семантической тройке — термин, от
              ...
              <th className="text-left py-1.5 px-2 text-muted-foreground font-normal">
                <div className="flex items-center gap-1">
                  ...
                  <TooltipIcon
                    content={
                      <p>
                        Тип семантической связи между субъектом и объектом.
                        Определяет характер отношения.
                      </p>
                    }
                    side="top"
                    align="start"
              ...
              <th className="text-left py-1.5 px-2 text-muted-foreground font-normal">
                <div className="flex items-center gap-1">
                  ...
                  <TooltipIcon
                    content={
                      <p>
                        Целевая сущность в семантической тройке — термин, на
                        который направлена связь.
                      </p>
                    }
                    side="top"
                    align="start"
              ...
              <th className="text-right py-1.5 px-2 text-muted-foreground font-normal">
                <div className="flex items-center justify-end gap-1">
                  ...
                  <TooltipIcon
                    content={
                      <p>
                        Числовая значимость связи от 0 до 1. Чем выше вес, тем
                        сильнее и достовернее отношение между сущностями.
                      </p>
                    }
                    side="top"
                    align="end"
          ...
          <tbody>
            {filtered.map((t, i) => (
              ...
              >
                <td className="py-1 px-2 text-foreground/90 max-w-[120px] truncate">
                  {t.subject}
                </td>
                <td className="py-1 px-2">
                  <div className="flex items-center gap-1">
                    ...
                    <TooltipIcon
                      content={RELATION_TOOLTIPS[t.predicate]}
                      side="top"
                      align="start"
                    />
                  </div>
