...
import { computeTFIDF, buildCoOccurrenceMatrix, extractEntityCandidates, extractRelationCandidates } from './lib/nlpEngine';
...
function runAnalysis(
  ...
  if (mode === 'global') {
    ...
    return { graph, triplets, taxonomy, ontology, intentGraphs: [], metaGraph: null };
  } else {
    const intentGraphs: KnowledgeGraph[] = intentGroups.map((group, i) => {
      ...
      const relations = extractRelationCandidates(coOccurrence, entities);
      return buildKnowledgeGraph(entities, relations, tfidfMap, `intent-${i}`, 80);
    });

    const intentLabels = intentGroups.map((g, i) => {
      const topQuery = g.sort((a, b) => b.frequency - a.frequency)[0]?.query ?? `Намерение ${i + 1}`;
    ...
    const globalRelations = extractRelationCandidates(globalCoOcc, globalEntities);
    const globalGraph = buildKnowledgeGraph(globalEntities, globalRelations, globalTfidf, 'global');
    ...
    return { graph: globalGraph, triplets, taxonomy, ontology, intentGraphs, metaGraph };
  }
}
...
const LEGEND_TOOLTIPS: Record<string, React.ReactNode> = {
  'association': (
    <div className="space-y-1">
      ...
      <p>Высокая совместная встречаемость и схожие TF-IDF значения. Термины тематически близки.</p>
    </div>
  ),
  'co-occurrence': (
    <div className="space-y-1">
      <p className="font-semibold" style={{ color: '#a855f7' }}>Совместная встречаемость</p>
      <p>Термины часто появляются вместе в пределах одного контекстного окна.</p>
    </div>
  ),
  'hierarchical': (
    <div className="space-y-1">
  ...
  'functional': (
    <div className="space-y-1">
      ...
      <p>Один термин модифицирует, уточняет или функционально дополняет другой.</p>
    </div>
  ),
};
...
export default function App() {
  ...
  const [selectedGraphIndex, setSelectedGraphIndex] = useState<number | 'meta'>(0);
  ...
  const displayGraph = useMemo(() => {
    ...
    if (typeof selectedGraphIndex === 'number') return intentGraphs[selectedGraphIndex] ?? null;
    return graph;
  ...
  const appId = typeof window !== 'undefined' ? encodeURIComponent(window.location.hostname) : 'knowledge-graph-builder';
  ...
  return (
    <div className="flex flex-col h-screen bg-kg-dark overflow-hidden">
      ...
      <header className="flex-shrink-0 flex items-center justify-between px-4 py-2.5 border-b border-kg-border bg-kg-panel-dark">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5">
            <div className="w-2 h-2 rounded-full bg-kg-cyan animate-pulse-glow" style={{ backgroundColor: 'oklch(0.82 0.19 195)' }} />
            <div className="w-2 h-2 rounded-full bg-kg-amber" style={{ backgroundColor: 'oklch(0.78 0.19 75)' }} />
            <div className="w-2 h-2 rounded-full bg-kg-purple" style={{ backgroundColor: 'oklch(0.65 0.22 300)' }} />
          </div>
          <h1 className="text-sm font-mono font-semibold tracking-tight" style={{ color: 'oklch(0.92 0.01 240)' }}>
            Построитель графа знаний
          </h1>
          <Badge variant="outline" className="text-xs font-mono border-kg-border text-muted-foreground hidden sm:flex">
            v1.0
          </Badge>
        </div>
        ...
        {stats && (
          <div className="flex items-center gap-4 text-xs font-mono text-muted-foreground">
            <span><span style={{ color: 'oklch(0.82 0.19 195)' }}>{stats.nodes}</span> узлов</span>
            <span><span style={{ color: 'oklch(0.78 0.19 75)' }}>{stats.edges}</span> рёбер</span>
            <span><span style={{ color: 'oklch(0.65 0.22 300)' }}>{stats.triplets}</span> триплетов</span>
          </div>
        )}
      </header>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="flex-shrink-0 w-52 flex flex-col gap-4 p-3 border-r border-kg-border bg-kg-panel-dark overflow-y-auto">
          ...
          <div className="space-y-1.5">
            <p className="text-xs font-mono text-muted-foreground uppercase tracking-widest">Источник данных</p>
          ...
          <button
            ...
            `}
            style={intentGroups.length > 0 && !isAnalyzing ? { color: 'oklch(0.82 0.19 195)', borderColor: 'oklch(0.82 0.19 195 / 0.6)' } : {}}
          >
            {isAnalyzing ? (
          ...
          {hasAnalyzed && (
            <>
              <Separator className="bg-kg-border" />
              <div className="space-y-1.5">
                <p className="text-xs font-mono text-muted-foreground uppercase tracking-widest">Легенда</p>
                <div className="space-y-1">
                  {[
                    { color: '#00d4f5', label: 'ассоциация', key: 'association' },
                    { color: '#a855f7', label: 'совм. встречаемость', key: 'co-occurrence' },
                    { color: '#f59e0b', label: 'иерархическая', key: 'hierarchical' },
                    { color: '#22c55e', label: 'функциональная', key: 'functional' },
                  ].map(({ color, label, key }) => (
                    <div key={key} className="flex items-center gap-2">
                      <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: color }} />
                      <span className="text-xs font-mono text-muted-foreground flex-1">{label}</span>
                      <TooltipIcon content={LEGEND_TOOLTIPS[key]} side="right" align="start" />
                    </div>
                  ))}
                </div>
              </div>
            ...
            <Tabs defaultValue="triplets" className="h-full flex flex-col">
              <div className="flex-shrink-0 px-3 pt-2">
                <TabsList className="bg-kg-panel border border-kg-border h-7">
                  <TabsTrigger value="triplets" className="text-xs font-mono h-5 px-3 data-[state=active]:bg-kg-cyan/20">
                    Триплеты
                    {triplets.length > 0 && (
                      <span className="ml-1.5 text-muted-foreground">({triplets.length})</span>
                    )}
                  </TabsTrigger>
                  <TabsTrigger value="taxonomy" className="text-xs font-mono h-5 px-3 data-[state=active]:bg-kg-amber/20">
                    Таксономия
                  </TabsTrigger>
                  <TabsTrigger value="ontology" className="text-xs font-mono h-5 px-3 data-[state=active]:bg-kg-purple/20">
                    Онтология
                    {ontology.length > 0 && (
                      <span className="ml-1.5 text-muted-foreground">({ontology.length})</span>
                    )}
                  </TabsTrigger>
                </TabsList>
              </div>

              <TabsContent value="triplets" className="flex-1 min-h-0 px-3 pb-2 mt-2 overflow-y-auto">
                <TripletsPanel triplets={triplets} />
              </TabsContent>
              <TabsContent value="taxonomy" className="flex-1 min-h-0 px-3 pb-2 mt-2 overflow-y-auto">
                <TaxonomyPanel taxonomy={taxonomy} />
              </TabsContent>
              <TabsContent value="ontology" className="flex-1 min-h-0 px-3 pb-2 mt-2 overflow-y-auto">
      ...
      <footer className="flex-shrink-0 flex items-center justify-center gap-1.5 px-4 py-1.5 border-t border-kg-border bg-kg-panel-dark text-xs font-mono text-muted-foreground">
        ...
        <Heart className="w-3 h-3 fill-current" style={{ color: 'oklch(0.82 0.19 195)' }} />
        <span>using</span>
        <a
