import { IntentGroup, QueryEntry } from '../types';
  ...
  for (let i = 0; i < cdCount; i++) {
    ...
    entries.set(name, {
      ...
      getData: async () => {
        ...
        const dataStart = capturedOffset + 30 + localFileNameLen + localExtraLen;
        const compressedData = buffer.slice(dataStart, dataStart + capturedCompressedSize);

        if (capturedMethod === 0) {
          // Stored (no compression)
          return compressedData;
        } else if (capturedMethod === 8) {
          // Deflate
...
export async function parseExcelFile(file: File): Promise<IntentGroup[]> {
  ...
  if (ssEntry) {
    ...
    siElements.forEach(si => {
      ...
      let value = '';
      tElements.forEach(t => { value += t.textContent ?? ''; });
  ...
  const rowMap = new Map<number, { colA: string | number | null; colB: string | number | null }>();

  const rowElements = sheetDoc.querySelectorAll('row');
  rowElements.forEach(rowEl => {
    const rowNum = parseInt(rowEl.getAttribute('r') ?? '0', 10) - 1; // 0-based
  ...
  const allRows: Array<{ colA: string | number | null; colB: string | number | null }> = [];
  ...
  for (const row of allRows) {
    ...
    const frequency = freqRaw !== '' ? Math.abs(parseInt(freqRaw, 10)) || 1 : 1;
