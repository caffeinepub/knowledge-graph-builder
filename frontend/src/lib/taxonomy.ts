import { KnowledgeGraph, TaxonomyNode } from '../types';
...
export function buildTaxonomy(graph: KnowledgeGraph): TaxonomyNode {
  ...
  const scored = graph.nodes.map(n => ({
    ...n,
    score: (n.tfidf / maxTfidf) * 0.6 + ((degree.get(n.id) ?? 0) / maxDegree) * 0.4,
  }));
  ...
  const nodeMap = new Map(scored.map(n => [n.id, n]));
...
export function flattenTaxonomy(
  ...
  depth: number = 0,
  parentLabel: string = ''
): Array<{ id: string; label: string; depth: number; parent: string; weight: number; tfidf: number }> {
  const result = [{ id: node.id, label: node.label, depth, parent: parentLabel, weight: node.weight, tfidf: node.tfidf }];
