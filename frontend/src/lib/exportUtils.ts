 ...
 */
function buildSheetXML(rows: SheetRow[], sharedStrings: Map<string, number>): string {
...
<sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" count="${count}" uniqueCount="${count}">`;
  ...
  const sorted = Array.from(sharedStrings.entries()).sort((a, b) => a[1] - b[1]);
...
function buildZip(files: Map<string, string>): Uint8Array {
  ...
  for (const [name, content] of files) {
    ...
    records.push({ name: nameBytes, data: dataBytes, crc, localHeaderOffset: offset });
...
export function exportToExcel(
  ...
  const sheets: Sheet[] = [
    {
      ...
      rows: [
        ...
        ...graph.nodes.map(n => [n.id, n.label, n.weight, n.tfidf, n.group, n.x, n.y, n.z]),
      ],
    },
    {
      name: 'Edges',
      rows: [
        ...
        ...graph.edges.map(e => [e.id, e.source, e.target, e.relationType, e.weight]),
      ],
    },
    {
      name: 'Triplets',
    ...
    },
    {
      ...
      rows: [
        ...
        ...flattenTaxonomy(taxonomy).map(t => [t.id, t.label, t.depth, t.parent, t.weight, t.tfidf]),
      ],
    },
    {
      name: 'Ontology',
      rows: [
        ...
        ...ontology.map(o => [o.entityA, o.entityB, o.relationType, o.confidence]),
      ],
    },
  ];
  ...
  sheets.forEach((sheet, i) => {
    files.set(`xl/worksheets/sheet${i + 1}.xml`, buildSheetXML(sheet.rows, sharedStrings));
  });
