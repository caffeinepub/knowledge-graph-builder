import { KnowledgeGraph, Triplet, OntologyEntry, RelationType } from '../types';
...
export function buildOntology(
  ...
  for (const edge of graph.edges) {
    ...
    if (nodeA.label.includes(nodeB.label) || nodeB.label.includes(nodeA.label)) {
...
export function ontologyToCSV(entries: OntologyEntry[]): string {
  ...
  const rows = entries
    .map(e => `"${e.entityA}","${e.entityB}","${e.relationType}",${e.confidence.toFixed(4)}`)
    .join('\n');
  return header + rows;
}
