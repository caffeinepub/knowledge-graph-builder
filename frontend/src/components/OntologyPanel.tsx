...
import { OntologyEntry, RelationType } from '../types';
...
const PANEL_TOOLTIP = (
  <div className="space-y-1.5">
    <p className="font-semibold" style={{ color: 'oklch(0.65 0.22 300)' }}>Онтология</p>
    <p>Формальное представление знаний: типизированные связи между сущностями с оценкой уверенности.</p>
    <ul className="space-y-0.5 list-none">
      <li><span style={{ color: 'oklch(0.82 0.19 195)' }}>Сущность 1/2</span> — связанные термины</li>
      <li><span style={{ color: 'oklch(0.78 0.19 75)' }}>Тип связи</span> — семантический тип отношения</li>
      <li><span style={{ color: 'oklch(0.65 0.22 300)' }}>Уверенность</span> — достоверность классификации (0–100%)</li>
    </ul>
    <p className="text-muted-foreground">Используйте фильтры для изучения конкретных типов отношений.</p>
  </div>
);

const CONFIDENCE_TOOLTIP = (
  <div className="space-y-1">
    <p className="font-semibold" style={{ color: 'oklch(0.65 0.22 300)' }}>Уверенность</p>
    <p>Степень достоверности классификации типа связи (0–100%). Высокое значение означает, что алгоритм уверен в правильности определённого типа отношения.</p>
    <ul className="space-y-0.5 list-none mt-1">
      <li><span style={{ color: 'oklch(0.72 0.2 145)' }}>&gt;70%</span> — высокая уверенность</li>
      <li><span style={{ color: 'oklch(0.78 0.19 75)' }}>40–70%</span> — средняя уверенность</li>
...
const FILTER_TOOLTIP = (
  <div className="space-y-1">
    <p className="font-semibold" style={{ color: 'oklch(0.65 0.22 300)' }}>Фильтр по типу связи</p>
    <p>Отображает только онтологические записи с выбранным типом отношения. «Все» — показывает все записи без фильтрации.</p>
  </div>
);
...
export function OntologyPanel({ ontology }: OntologyPanelProps) {
  ...
  const filtered = useMemo(() => {
    return filterType === 'all' ? ontology : ontology.filter(e => e.relationType === filterType);
  }, [ontology, filterType]);
          ...
          <thead>
            <tr className="border-b border-kg-border">
              <th className="text-left py-1.5 px-2 text-muted-foreground font-normal">
                <div className="flex items-center gap-1">
                  Сущность 1
                  <TooltipIcon content={<p>Первая сущность в паре. Субъект онтологического отношения.</p>} side="top" align="start" />
                </div>
              </th>
              <th className="text-left py-1.5 px-2 text-muted-foreground font-normal">
                <div className="flex items-center gap-1">
                  Тип связи
                  <TooltipIcon content={<p>Семантический тип отношения между двумя сущностями. Определяет характер связи.</p>} side="top" align="start" />
                </div>
              </th>
              <th className="text-left py-1.5 px-2 text-muted-foreground font-normal">
                <div className="flex items-center gap-1">
                  Сущность 2
                  <TooltipIcon content={<p>Вторая сущность в паре. Объект онтологического отношения.</p>} side="top" align="start" />
                </div>
              </th>
              <th className="text-right py-1.5 px-2 text-muted-foreground font-normal">
                <div className="flex items-center justify-end gap-1">
                  Уверенность
                  <TooltipIcon content={CONFIDENCE_TOOLTIP} side="top" align="end" />
                </div>
              </th>
            </tr>
          </thead>
          <tbody>
            {filtered.map((e, i) => (
              <tr key={i} className="border-b border-kg-border/40 hover:bg-kg-panel/50 transition-colors">
                <td className="py-1 px-2 text-foreground/90 max-w-[110px] truncate">{e.entityA}</td>
                <td className="py-1 px-2">
                  <div className="flex items-center gap-1">
                    ...
                    <TooltipIcon content={RELATION_TOOLTIPS[e.relationType]} side="top" align="start" />
                  </div>
                </td>
                <td className="py-1 px-2 text-foreground/90 max-w-[110px] truncate">{e.entityB}</td>
                <td className="py-1 px-2 text-right">
                  <span
                    className="font-mono"
                    style={{
                      color: e.confidence > 0.7
                        ? 'oklch(0.72 0.2 145)'
                        : e.confidence > 0.4
                        ? 'oklch(0.78 0.19 75)'
                        : 'oklch(0.55 0.02 240)',
